 ...
 *   - Manual setup for any HTTPS git remote (GitLab, Gitea, Bitbucket, etc.)
 *
...
class GitSyncService {
   ...
   */
  private async exportToFiles(repos: ReturnType<GitSyncService['createRepos']>): Promise<void> {
   ...
   *
   * @param remoteUrl - HTTPS URL of the remote repo.
   * @param branch    - Branch to clone.
   * @param token     - Auth token for HTTPS authentication.
   */
  private async cloneRepo(remoteUrl: string, branch: string, token: string): Promise<void> {
  ...
  async getStatus(): Promise<GitSyncStatus> {
    ...
    const connected = config !== undefined && token !== null && this.isCloned()
  ...
  async connectGitHub(): Promise<GitSyncStatus> {
    ...
    await this.cloneRepo(repo.clone_url, branch, token)
   ...
   *   branch name, and the auth token for HTTPS authentication.
   * @returns The updated sync status after a successful connection.
  ...
  async connectManual(input: ManualGitConfig): Promise<GitSyncStatus> {
    ...
    await this.cloneRepo(input.remoteUrl, branch, input.authToken)

    await this.storeToken(input.authToken)
    const config: GitSyncConfig = {
      provider: 'generic',
  ...
  async push(): Promise<GitPushResult> {
    ...
    const token = await this.getToken()
    if (!token) throw new Error('Git sync auth token is missing. Reconnect to continue.')
    ...
    try {
      await this.exportToFiles(repos)
      ...
      repos.log.insert({
        ...
        details: { commitHash: commitHash ?? 'no-op', remoteUrl: config.remoteUrl },
      })

      return { success: true, ...(commitHash !== undefined ? { commitHash } : {}) }
    } catch (err) {
